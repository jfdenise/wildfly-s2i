...
Feature: Wildfly Legacy s2i tests
  ...
  Scenario: Test provisioning.xml file
    Given s2i build https://github.com/jfdenise/wildfly-s2i from test/vanilla-wildfly/test-app-local-provisioning with env and True using wf-glow-test-apps
      | variable                             | value         |
...
Scenario: Test preconfigure.sh
    Given s2i build https://github.com/wildfly/wildfly-s2i from test/test-app-advanced-extensions with env and True using legacy-s2i-images
      ...
      | GALLEON_PROVISION_FEATURE_PACKS | org.wildfly:wildfly-galleon-pack:31.0.0.Beta1, org.wildfly.cloud:wildfly-cloud-galleon-pack:5.0.2.Final |
    Then container log should contain WFLYSRV0025
 ...
 Scenario: Test invalid layer
  ...
  Scenario: Test preview FP and preview cloud FP with legacy app.
    Given s2i build https://github.com/wildfly/wildfly-s2i from test/test-app with env and True using legacy-s2i-images
      | variable                             | value         |
      | GALLEON_PROVISION_LAYERS | cloud-server |
      | GALLEON_PROVISION_FEATURE_PACKS | org.wildfly:wildfly-preview-feature-pack:31.0.0.Beta1, org.wildfly.cloud:wildfly-preview-cloud-galleon-pack:5.0.2.Final |
   Then container log should contain WFLYSRV0025
    And container log should contain WFLYSRV0010: Deployed "ROOT.war"
    And check that page is served
      | property | value |
...
Scenario: Test external driver created during s2i.
  ...
  Scenario: Multiple deployments legacy
   Given s2i build http://github.com/jfdenise/wildfly-s2i from test/test-app-multi-deployments-legacy with env and True using wf-glow-test-apps
   | variable                 | value           |
